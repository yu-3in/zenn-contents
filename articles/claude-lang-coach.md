...
published: true
published_at: 2026-04-14 08:00
---

こんにちは、[yuuumin](https://x.com/yu_3in) です。

PR の description を英語で書いて Submit する。「...これ合ってるのかな」と思いつつ、誰にも確認できないからそのまま。Duolingo は 3 ヶ月で通知をオフにした。オンライン英会話は業務と関係ない例文ばかりで続かなかった。

エンジニアって毎日英語に囲まれているのに、なぜか英語ができるようにならないんですよね。

ほしいのは英語教室じゃなくて、隣にネイティブの同僚がいて、自分の書いた文章をさりげなく直してくれる環境。考えてみれば、普段の開発で Claude Code に日本語や英語のテキストを毎日打ち込んでいる。これをそのまま添削してもらえたら、もうネイティブの同僚が隣にいるのと同じなんじゃないか。

そう思って、Claude Code のプラグインを作りました。
...
_日本語で指示を出すと、英語ではこう表現する、というフィードバックが自動で付く_
