---
title: "Next.js × Convex x Clerkでフルスタックアプリをラクに開発する"
emoji: "🔨"
...
この記事では、Next.js x Convex x Clerk でフルスタックアプリを作る方法をハンズオン形式で紹介します。
...
つづいて、Convex をセットアップします。新しいターミナルを開いて、以下のコマンドを実行します。（ `npm run dev` は終了させないでください）
...
`app`ディレクトリと同階層に`providers`ディレクトリを作成します。続いて、`providers`ディレクトリに`convex-client-provider.ts`を作成します。
...
続いて、`convex/api.ts`を作成します。ここでは、`messages`テーブルの CRUD API を作成します。
...
:::details ここまでの `convex/api.ts` のコードは、以下のようになります。
